...
type Question = { id: string; kind: QuestionKind; text: string; equation?: string; options: string[]; answer: number; answers?: number[]; statements?: TrueFalseStatement[]; expected?: string; pairs?: { left: string; right: string }[]; points: number; responseLines?: number; showResponseLines?: boolean };
...
async function renderMathCanvas(source: string, widthMm: number, fontSizePt: number, bold: boolean, fontFamily: string) {
...
const trueFalseInstructions = "Lee cada afirmación y escribe V o F en el espacio.";
function createTrueFalseStatements(): TrueFalseStatement[] {
  return Array.from({ length: 4 }, (_, index): TrueFalseStatement => ({
    id: uid(),
    text: "",
    answer: index % 2 === 0 ? "V" : "F",
  }));
}
const questionTypes: { kind: QuestionKind; label: string; group: string; description: string }[] = [
  ...
  { kind: "truefalse", label: "Verdadero o falso", group: "Selección", description: "Varias afirmaciones con espacio para V/F" },
...
function parseQuestions(value: unknown): Question[] | null {
  ...
  for (const candidate of value) {
    ...
    const rawStatements = candidate.statements;
    ...
    let text = legacyDisplayEquation?.text ?? rawText;
    const equation = typeof candidateEquation === "string" ? candidateEquation : legacyDisplayEquation?.equation;
    if ((kind === "choice" || kind === "multiple") && options.length < 2) return null;
    let answer = 0;
    if (kind === "choice") {
    ...
    let statements: TrueFalseStatement[] | undefined;
    if (kind === "truefalse") {
      if (rawStatements === undefined) {
        const candidateAnswer = candidate.answer;
        if (options.length !== 2 || typeof candidateAnswer !== "number" ||
            !Number.isInteger(candidateAnswer) || candidateAnswer < 0 || candidateAnswer >= options.length) return null;
        const oldAnswer = options[candidateAnswer].trim().toLowerCase();
        const correctAnswer = oldAnswer === "f" || oldAnswer === "falso" ? "F"
          : oldAnswer === "v" || oldAnswer === "verdadero" ? "V"
          : candidateAnswer === 0 ? "V" : "F";
        statements = [{ id: uid(), text, answer: correctAnswer }];
        text = trueFalseInstructions;
      } else {
        if (!Array.isArray(rawStatements) || !rawStatements.length || rawStatements.length > 100) return null;
        statements = [];
        for (const statement of rawStatements) {
          if (!isRecord(statement) || typeof statement.text !== "string" || statement.text.length > 10_000 ||
              statement.answer !== "V" && statement.answer !== "F") return null;
          statements.push({ id: uid(), text: statement.text, answer: statement.answer });
        }
      }
    }
    ...
    questions.push({
      ...
      text,
      equation,
      options: kind === "truefalse" ? [] : [...options],
      answer,
      answers,
      statements,
...
export default function Home() {
  ...
  const [pdfAllowQuestionSplits, setPdfAllowQuestionSplits] = useState(false);
  const [pdfPreviewUrl, setPdfPreviewUrl] = useState("");
  const [headerPdfPreviewUrl, setHeaderPdfPreviewUrl] = useState("");
  const [headerPreviewBusy, setHeaderPreviewBusy] = useState(false);
  const [headerPreviewError, setHeaderPreviewError] = useState("");
  const [headerPreviewWidth, setHeaderPreviewWidth] = useState(480);
  const [headerPreviewRevision, setHeaderPreviewRevision] = useState(0);
  const [headerPreviewVisible, setHeaderPreviewVisible] = useState(true);
  const [railExpanded, setRailExpanded] = useState(true);
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const [mobileActionsOpen, setMobileActionsOpen] = useState(false);
  const headerEditorLayoutRef = useRef<HTMLDivElement>(null);
  const headerPreviewResizeRef = useRef<{ pointerId: number; startX: number; width: number } | null>(null);
  const headerPreviewPlanRef = useRef<{ sections: Section[]; variants: number; forms: FormPlan[] } | null>(null);
  const headerPreviewRequestRef = useRef(0);
  const headerPreviewQueueRef = useRef<Promise<void>>(Promise.resolve());
  ...
  useEffect(() => {
    ...
    updateRailForViewport();
    window.addEventListener("resize", updateRailForViewport);
  ...
  useEffect(() => {
    ...
    if (tab !== "header" || modal === "export" || !headerPreviewVisible) {
      ...
      setHeaderPreviewBusy(false);
      return () => {
        cancelled = true;
        if (headerPreviewRequestRef.current === request) headerPreviewRequestRef.current++;
      };
    }
    setHeaderPreviewBusy(true);
    setHeaderPreviewError("");
    const timer = window.setTimeout(() => {
      const render = headerPreviewQueueRef.current.catch(() => undefined).then(async () => {
        if (cancelled || headerPreviewRequestRef.current !== request) return;
        try {
          const blob = await exportForms(planHeaderPreviewForms(), false, pdfStyle);
          if (cancelled || headerPreviewRequestRef.current !== request) return;
          setHeaderPdfPreviewUrl(URL.createObjectURL(blob));
        } catch {
          if (!cancelled && headerPreviewRequestRef.current === request) setHeaderPreviewError("No se pudo generar la vista previa del PDF.");
        } finally {
          if (!cancelled && headerPreviewRequestRef.current === request) setHeaderPreviewBusy(false);
        }
      });
      headerPreviewQueueRef.current = render;
    }, 500);
    return () => {
      cancelled = true;
  ...
  function addQuestion(si: number, kind: QuestionKind = "choice") {
    const options = ["choice", "multiple"].includes(kind) ? ["Alternativa A", "Alternativa B", "Alternativa C", "Alternativa D"] : [];
    const statements = kind === "truefalse" ? createTrueFalseStatements() : undefined;
    const q: Question = {
      id: uid(),
      kind,
      text: kind === "truefalse" ? trueFalseInstructions : "Escribe tu pregunta aquí…",
      equation: "",
      options,
      answer: 0,
      answers: kind === "multiple" ? [0] : [],
      statements,
      expected: "",
      pairs: kind === "matching" ? [{ left: "Concepto A", right: "Definición A" }, { left: "Concepto B", right: "Definición B" }] : [],
      points: statements?.length || 1,
      responseLines: 3,
      showResponseLines: kind === "written" ? true : undefined,
    };
  ...
  function changeQuestionKind(si: number, qi: number, kind: QuestionKind) {
    ...
    if (!current) return;
    const isNewTrueFalse = kind === "truefalse" && current.kind !== "truefalse";
    const carriedStatement = isNewTrueFalse && current.text.trim() !== "" && current.text !== "Escribe tu pregunta aquí…";
    const statements = kind !== "truefalse" ? undefined
      : current.kind === "truefalse" ? current.statements || createTrueFalseStatements()
      : [
        ...(carriedStatement ? [{ id: uid(), text: current.text, answer: "V" as const }] : []),
        ...createTrueFalseStatements().slice(carriedStatement ? 1 : 0),
      ];
    const text = kind === "truefalse"
      ? current.kind === "truefalse" ? current.text : trueFalseInstructions
      : current.kind === "truefalse"
        ? (current.statements || []).map(statement => statement.text).filter(Boolean).join("\n") || "Escribe tu pregunta aquí…"
        : current.text;
    const options = ["choice", "multiple"].includes(kind) ? ["Alternativa A", "Alternativa B", "Alternativa C", "Alternativa D"] : [];
  ...
  function planHeaderPreviewForms() {
    const cached = headerPreviewPlanRef.current;
    if (cached?.sections === exam.sections && cached.variants === exam.variants) return cached.forms;
    const forms = planForms();
    headerPreviewPlanRef.current = { sections: exam.sections, variants: exam.variants, forms };
    return forms;
  }
  function headerPreviewIsCompact() {
    return typeof window !== "undefined" && window.innerWidth <= 1280;
  }
  function headerPreviewWidthMinimum() {
    return headerPreviewIsCompact() ? 320 : 360;
  }
  function headerPreviewWidthMaximum() {
    const compact = headerPreviewIsCompact();
    const viewportWidth = typeof window === "undefined" ? 1440 : window.innerWidth;
    const minPreview = compact ? 320 : 360;
    const minEditor = compact ? 360 : 560;
    const available = headerEditorLayoutRef.current?.clientWidth ?? viewportWidth - 100;
    return Math.max(minPreview, available - minEditor - 28);
  }
  function startHeaderPreviewResize(event: React.PointerEvent<HTMLDivElement>) {
    if (event.button !== 0) return;
    event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    headerPreviewResizeRef.current = { pointerId: event.pointerId, startX: event.clientX, width: headerPreviewWidth };
  }
  function moveHeaderPreviewResize(event: React.PointerEvent<HTMLDivElement>) {
    const drag = headerPreviewResizeRef.current;
    if (!drag || drag.pointerId !== event.pointerId) return;
    const maximum = headerPreviewWidthMaximum();
    setHeaderPreviewWidth(Math.max(headerPreviewWidthMinimum(), Math.min(maximum, Math.round(drag.width + drag.startX - event.clientX))));
  }
  function endHeaderPreviewResize() {
    headerPreviewResizeRef.current = null;
  }
  function handleHeaderPreviewResizeKey(event: React.KeyboardEvent<HTMLDivElement>) {
    if (event.key !== "ArrowLeft" && event.key !== "ArrowRight") return;
    event.preventDefault();
    const maximum = headerPreviewWidthMaximum();
    setHeaderPreviewWidth(width => Math.max(headerPreviewWidthMinimum(), Math.min(maximum, width + (event.key === "ArrowLeft" ? 32 : -32))));
  }
  async function exportForms(forms: FormPlan[], answerKey = false, styleName: PdfStyle = pdfStyle, allowQuestionSplits = pdfAllowQuestionSplits, fontName: PdfFont = pdfFont): Promise<Blob> {
    const pdf = new jsPDF({ unit: "mm", format: "letter" });
    const style = {
      compact: { margin: 13, top: 12, bottomInset: 14, scale: 0.88, leading: 0.4, paragraphGap: 0.5, questionGap: 1, questionTopMargin: 1.5, fieldRow: 6, fieldFont: 7, titleFont: 12 },
      balanced: { margin: 18, top: 16, bottomInset: 19, scale: 1, leading: 0.48, paragraphGap: 1.2, questionGap: 2, questionTopMargin: 1.5, fieldRow: 7.5, fieldFont: 8, titleFont: 14 },
      spacious: { margin: 23, top: 21, bottomInset: 25, scale: 1.08, leading: 0.56, paragraphGap: 2, questionGap: 3, questionTopMargin: 0, fieldRow: 9.5, fieldFont: 8.5, titleFont: 16 },
    }[styleName];
    const mathFontFamily = fontName === "times" ? '"Times New Roman", serif' : fontName === "courier" ? '"Courier New", monospace' : "Arial, sans-serif";
    ...
    for (let form = 0; form < forms.length; form++) {
      if (form) pdf.addPage();
      const formFirstPage = pdf.getNumberOfPages();
      let y = top;
      ...
      const write = async (text: string, size = 10, bold = false) => {
        ...
        if (containsLatexMath(text)) {
          ...
          if (y > top && y + canvas.height / pixelsPerMm > bottom) newContentPage();
          if (y + canvas.height / pixelsPerMm <= bottom) {
            pdf.addImage(canvas, "PNG", left, y, width, canvas.height / pixelsPerMm, undefined, "FAST");
            y += canvas.height / pixelsPerMm + Math.max(style.paragraphGap, actualSize * 0.2);
            return;
          }
          let sourceY = 0;
        ...
        const lines = pdf.splitTextToSize(normalizedText, width) as string[];
      ...
      const answerKeyText = (item: PlannedQuestion, questionNumber: number) => {
        ...
        if (q.kind === "truefalse") key = `V/F: ${(q.statements || []).map((statement, index) => `${index + 1}-${statement.answer}`).join(" · ") || "(sin afirmaciones)"}`;
        ...
        if (q.kind === "choice") key = String.fromCharCode(65 + item.optionOrder.indexOf(q.answer));
        if (q.kind === "matching") key = (q.pairs || []).map((_, index) => `${index + 1}–${String.fromCharCode(65 + item.matchOrder.indexOf(index))}`).join(" · ");
        return `${questionNumber}. ${key} (${q.points} pto${q.points === 1 ? "" : "s"})`;
      };
      const measureQuestionHeight = async (item: PlannedQuestion, questionNumber: number) => {
        const q = item.question;
        let height = style.questionTopMargin;
        if (answerKey) return height + await measureTextHeight(answerKeyText(item, questionNumber), 10) + style.questionGap;
        height += await measureTextHeight(`${questionNumber}. ${q.text}${q.kind === "multiple" ? " (Marca todas las alternativas correctas; puede haber más de una.)" : ""} (${q.points} pto${q.points === 1 ? "" : "s"})`, 10, true);
        if (q.equation?.trim()) height += style.questionGap + await measureTextHeight(`\\[${q.equation.trim()}\\]`, 10) + Math.max(2, style.questionGap);
        if (q.kind === "written") {
          height += style.questionGap + (q.showResponseLines !== false ? Math.max(1, Math.min(12, q.responseLines ?? 3)) * (styleName === "spacious" ? 9 : styleName === "balanced" ? 7 : 5.5) : 0);
        } else if (q.kind === "fill") {
          ...
          for (const [displayIndex, index] of item.optionOrder.entries()) height += await measureTextHeight(`${String.fromCharCode(65 + displayIndex)})  ${q.options[index] || ""}`, 9);
        }
        return height + style.questionGap;
      };
      const drawBlock = (
        ...
        for (const line of lines) {
          ...
          const wrapped = pdf.splitTextToSize(normalizedText, blockWidth) as string[];
          const lineHeight = Math.max(3.2, actualSize * style.leading);
      ...
      for (const section of forms[form].sections) {
        ...
        for (const item of section.questions) {
          const q = item.question;
          number++;
          if (!allowQuestionSplits) {
            const questionHeight = await measureQuestionHeight(item, number);
            if (y > top && y + questionHeight > bottom) newContentPage();
          }
          y += style.questionTopMargin;
          if (answerKey) {
            await write(answerKeyText(item, number), 10);
          } else {
      ...
      const formLastPage = pdf.getNumberOfPages();
      for (let pageNumber = formFirstPage; pageNumber <= formLastPage; pageNumber++) {
        pdf.setPage(pageNumber);
        pdf.setFont(fontName, "normal");
        pdf.setFontSize(7 * style.scale);
        pdf.setTextColor(90);
        const footerLabel = `${answerKey ? "PAUTA" : "APOLLO"}  •  Forma ${String.fromCharCode(65 + form)}`;
        pdf.text(footerLabel, left, footerY);
        if (exam.header.repeatOnPages && pageNumber > formFirstPage) {
          pdf.setFontSize(7.2 * style.scale);
          const footerWidth = Math.max(20, width - pdf.getTextWidth(footerLabel) - 8);
          const footerLines = pdf.splitTextToSize(repeatedFooterText.replace(/→/g, "->"), footerWidth) as string[];
          const lineHeight = Math.max(3.2, 7.2 * style.scale * style.leading);
          pdf.text(footerLines, left + width, footerY - (footerLines.length - 1) * lineHeight, { align: "right" });
        }
      }
    }
  ...
  return <main className={railExpanded ? "shell rail-expanded" : "shell"}>
    <aside className="rail" aria-label="Navegación principal">
      ...
      <button type="button" className="rail-toggle" aria-label={railExpanded ? "Contraer navegación" : "Expandir navegación"} aria-expanded={railExpanded} title={railExpanded ? "Contraer navegación" : "Expandir navegación"} onClick={() => setRailExpanded(expanded => !expanded)}>
        <span aria-hidden="true">{railExpanded ? "‹" : "›"}</span><span className="rail-toggle-label">{railExpanded ? "Contraer" : "Expandir"}</span>
      </button>
    ...
    <section className="workspace"><header className="topbar">
      ...
      <div className={mobileActionsOpen ? "top-actions actions-open" : "top-actions"} id="top-actions-menu" onClick={() => setMobileActionsOpen(false)}>
        ...
        {!user && <button className="btn ghost" onClick={() => setModal("auth")}>Crear cuenta</button>}
        <button className="btn ghost forms-button" onClick={() => setModal("variants")}>Formas: {exam.variants}</button>
        <button className="btn ghost" onClick={save}>{busy === "save" ? "Guardando…" : "Guardar"}</button>
        <button className="btn primary" onClick={printPdf}>{busy === "pdf" ? "Preparando vista previa…" : <><span>↓</span> Exportar PDF</>}</button>
        <button className="btn ghost" onClick={printAnswerKey} disabled={busy === "key" || !generation || generation.signature !== JSON.stringify(exam)} title={!generation || generation.signature !== JSON.stringify(exam) ? "Exporta las formas actuales antes de descargar la pauta" : "Descargar pauta"}>{busy === "key" ? "Generando pauta…" : "Descargar pauta"}</button>
      </div>
    </header>
      ...
      {tab === "editor" && <div className="content editor-layout"><section className="edit-column"><div className="eyebrow"><span>✦</span> CREADOR DE PRUEBAS</div><input className="exam-title" value={exam.title} onChange={e => updateExam({ title: e.target.value })} aria-label="Título de la prueba"/><div className="metadata-grid"><label>ASIGNATURA<input value={exam.subject} onChange={e => updateExam({ subject: e.target.value })}/></label><label>NIVEL / CURSO<input value={exam.grade} onChange={e => updateExam({ grade: e.target.value })}/></label><label>DOCENTE<input value={exam.teacher} placeholder="Tu nombre" onChange={e => updateExam({ teacher: e.target.value })}/></label><label>DURACIÓN<input value={exam.duration} onChange={e => updateExam({ duration: e.target.value })}/></label></div>
          ...
          {exam.sections[active] ? <div className="section-card" id="section-editor"><div className="section-head"><div><div className="eyebrow">SECCIÓN {String(active + 1).padStart(2, "0")}</div><input className="section-title" aria-label="Título de la sección" value={exam.sections[active].title} onChange={e => updateSection(active, { title: e.target.value })}/></div><button type="button" className="icon-btn danger-icon" title="Eliminar sección" aria-label="Eliminar sección" onClick={() => deleteSection(active)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3m3 0-1 13H7L6 7m4 4v5m4-5v5" /></svg></button></div><textarea className="instructions" aria-label="Instrucciones de la sección" value={exam.sections[active].instructions} onChange={e => updateSection(active, { instructions: e.target.value })} placeholder="Instrucciones para esta sección"/><div className="pick-row"><span>Preguntas disponibles <b>{exam.sections[active].questions.length}</b></span><label>{exam.variants > 1 ? "Preguntas por forma" : "Preguntas incluidas"} <input type="number" min="0" max={exam.sections[active].questions.length} value={Math.max(0, Math.min(exam.sections[active].pick ?? 0, exam.sections[active].questions.length))} onChange={e => updateSection(active, { pick: Math.max(0, Math.min(Number(e.target.value) || 0, exam.sections[active].questions.length)) })} /></label></div>
            {exam.sections[active].questions.map((q, qi) => <article className="question-card" key={q.id} data-include-response-lines={q.showResponseLines !== false}>
              <div className="question-toolbar"><select aria-label="Tipo de pregunta" value={q.kind} onChange={e => changeQuestionKind(active, qi, e.target.value as QuestionKind)}>{questionTypes.map(type => <option key={type.kind} value={type.kind}>{type.label}</option>)}</select><label className="points">{q.kind === "truefalse" ? "Puntaje total" : "Puntaje"} <input type="number" min="0" value={q.points} onChange={e => updateQuestion(active, qi, { points: Math.max(0, Number(e.target.value) || 0) })}/></label><button className="delete-q" title="Eliminar pregunta" aria-label="Eliminar pregunta" onClick={() => { const section = exam.sections[active]; const questions = section.questions.filter((_, i) => i !== qi); updateSection(active, { questions, pick: Math.min(section.pick, questions.length) }); }}>×</button></div>
              <textarea className="question-prompt" aria-label={q.kind === "truefalse" ? "Indicaciones para las afirmaciones" : "Enunciado de la pregunta"} value={q.text} onChange={event => updateQuestion(active, qi, { text: event.target.value })} placeholder={q.kind === "fill" ? "Incluye ____ por cada espacio que deben completar" : q.kind === "truefalse" ? trueFalseInstructions : "Escribe el enunciado de la pregunta"}/>
              {q.kind === "truefalse" && <div className="truefalse-editor" aria-label="Afirmaciones verdadero o falso">
                <div className="truefalse-editor-heading"><div className="eyebrow">AFIRMACIONES V / F</div><span>Escribe una afirmación por fila y selecciona su respuesta para la pauta. El puntaje de este bloque es el total indicado arriba.</span></div>
                {(q.statements || []).map((statement, statementIndex) => <div className="truefalse-statement-row" key={statement.id}>
                  <span aria-hidden="true">{statementIndex + 1}.</span>
                  <textarea aria-label={`Afirmación ${statementIndex + 1}`} maxLength={10_000} value={statement.text} onChange={event => updateQuestion(active, qi, { statements: (q.statements || []).map((item, index) => index === statementIndex ? { ...item, text: event.target.value } : item) })} placeholder={`Escribe la afirmación ${statementIndex + 1}`}/>
                  <label>Correcta<select aria-label={`Respuesta correcta de la afirmación ${statementIndex + 1}`} value={statement.answer} onChange={event => updateQuestion(active, qi, { statements: (q.statements || []).map((item, index) => index === statementIndex ? { ...item, answer: event.target.value as "V" | "F" } : item) })}>
                    <option value="V">V · Verdadero</option><option value="F">F · Falso</option>
                  </select></label>
                  <button type="button" className="option-delete" aria-label={`Eliminar afirmación ${statementIndex + 1}`} disabled={(q.statements || []).length <= 1} onClick={() => updateQuestion(active, qi, { statements: (q.statements || []).filter((_, index) => index !== statementIndex) })}>×</button>
                </div>)}
                <button type="button" className="add-option" disabled={(q.statements || []).length >= 100} onClick={() => updateQuestion(active, qi, { statements: [...(q.statements || []), { id: uid(), text: "", answer: "V" }] })}>＋ Añadir afirmación</button>
              </div>}
              <div className="question-math-controls">
                ...
                {q.equation?.trim() && <button type="button" className="btn ghost math-equation-remove" aria-label="Quitar ecuación de la pregunta" onClick={() => { updateQuestion(active, qi, { equation: "" }); if (activeMathQuestion === q.id) setActiveMathQuestion(null); }}>Quitar ecuación</button>}
              </div>
      ...
      {tab === "header" && <div className="content editor-layout header-editor-layout" data-preview-visible={headerPreviewVisible ? "true" : "false"} ref={headerEditorLayoutRef} style={{ "--header-preview-width": `${headerPreviewWidth}px` } as React.CSSProperties}>
        <section className="edit-column">
          ...
          <div className="header-title-row">
            ...
            <button type="button" className="btn ghost header-preview-toggle" aria-expanded={headerPreviewVisible} onClick={() => setHeaderPreviewVisible(visible => !visible)}>{headerPreviewVisible ? "Ocultar vista previa" : "Mostrar vista previa"}</button>
          </div>
          ...
          <div className="header-config-card">
            ...
            <label className="header-repeat-option">
              ...
              <span><b>Identificación en páginas siguientes</b><small>Mostrar el título y la forma al pie, en el lado derecho.</small></span>
            </label>
        ...
        {headerPreviewVisible && <>
        <div
          className="header-preview-resizer"
          role="separator"
          aria-label="Cambiar el ancho del PDF de vista previa"
          aria-orientation="vertical"
          aria-valuemin={320}
          aria-valuemax={960}
          aria-valuenow={headerPreviewWidth}
          aria-valuetext={`${headerPreviewWidth} píxeles`}
          tabIndex={0}
          onPointerDown={startHeaderPreviewResize}
          onPointerMove={moveHeaderPreviewResize}
          onPointerUp={endHeaderPreviewResize}
          onPointerCancel={endHeaderPreviewResize}
          onLostPointerCapture={endHeaderPreviewResize}
          onKeyDown={handleHeaderPreviewResizeKey}
        />
        <aside className="header-preview-column" aria-label="Vista previa del PDF real">
          <div className="header-preview-card header-pdf-card">
            <div className="header-pdf-heading">
              <div>
                <div className="eyebrow">VISTA PREVIA EN VIVO · PDF REAL</div>
                <p>El documento impreso, con los cambios actuales.</p>
              </div>
              {headerPdfPreviewUrl && <a className="btn ghost header-open-pdf" href={headerPdfPreviewUrl} target="_blank" rel="noreferrer">Abrir aparte</a>}
            </div>
            <div className="header-pdf-meta" role="status" aria-live="polite">
              {headerPreviewError || (headerPreviewBusy ? "Actualizando PDF…" : headerPdfPreviewUrl ? "PDF actualizado · carta" : "Generando PDF…")}
            </div>
            <div className="header-pdf-frame" aria-busy={headerPreviewBusy}>
              {headerPdfPreviewUrl
                ? <iframe title="Vista previa del PDF real" src={`${headerPdfPreviewUrl}#toolbar=0`}/>
                : headerPreviewError
                  ? <div className="header-pdf-placeholder header-pdf-error" role="alert">{headerPreviewError}<button type="button" className="btn ghost" onClick={() => setHeaderPreviewRevision(revision => revision + 1)}>Reintentar</button></div>
                  : <div className="header-pdf-placeholder" role="status">Generando PDF real…</div>}
              {headerPreviewBusy && headerPdfPreviewUrl && <div className="header-pdf-updating">Actualizando PDF…</div>}
            </div>
            <div className="header-preview-caption">Vista idéntica al PDF de estudiantes. Arrastra el divisor para cambiar el ancho; también puedes redimensionar el alto.</div>
            <div className="header-preview-actions"><button className="btn primary" onClick={printPdf}>Abrir vista completa</button></div>
          </div>
        </aside>
        </>}
      </div>}
    ...
    {modal && (
      <div className="modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) setModal(null); }}>
        <div
          ...
          ref={dialogRef}
        >
          <button className="modal-close" aria-label="Cerrar diálogo" onClick={() => setModal(null)}>×</button>
          {modal === "auth" ? (
            <>
              <div className="brand-large">A<span>.</span></div>
              <div className="eyebrow">BIENVENIDO A APOLLO</div>
              <h2>{authMode === "register" ? "Tu próxima prueba empieza aquí." : "Qué bueno verte de nuevo."}</h2>
            ...
            <>
              ...
              <p>Genera hasta 10 versiones. El orden de las preguntas y alternativas se mezcla por forma.</p>
              <label className="variant-number">Cantidad de formas<input type="number" min="1" max="10" value={exam.variants} onChange={event => updateExam({ variants: Math.max(1, Math.min(10, Number(event.target.value))) })}/></label>
